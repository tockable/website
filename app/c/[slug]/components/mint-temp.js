...
const initialArgs = {
  args: [1, [[]], EMPTY_BYTES_32, 0, [[EMPTY_BYTES_32]]],
  value: 0,
};

export default function MintTemp({
...
function MintHandler({ role, prepareMint, session }) {
        ...
        await storeMinted(
          {
            ...
            dropType:
              project.dropType === "temp" ? "tockable" : project.dropType,
            amount: blobs.length,
            timeStamp,
          },
          ref
  ...
  async function mint() {
    ...
    const fee = parseEther(
      ((Number(role.price) + getBaseFee(project)) * blobs.length).toString(),
      "wei"
    );
...
function getBaseFee(project) {
  return Number(
    project.chainData[
      project.dropType === "temp" ? "base_fee" : "regular_base_fee"
    ]
  );
}
