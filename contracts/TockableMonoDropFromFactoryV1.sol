...
contract TockableMonoDropFromFactoryV1 is ERC721AQueryable, Ownable(msg.sender), ReentrancyGuard {
    /**
     ************
     * Contract *
     ************
    ...
    */
    uint256 public constant DROP_TYPE = 6;
    ...
    function publicMint(uint256 _quantity) external payable nonReentrant {
        if (!isMintLive) revert MintIsNotLive();
        require(activeSession == 0, "Can't mint at this session");

        isTokenLeftInTotal(_quantity);
        isTokenLeftInActiveSession(_quantity);
        isTokenLeftForAddressInRoleInSession(msg.sender, 0, _quantity);

        uint256 payAmount = (getRoleById[0].price + BASE_FEE) * _quantity;
        if (msg.value < payAmount) revert NotEnoughEth();

        _safeMint(msg.sender, _quantity);

        mintedInSessionById[0] = mintedInSessionById[0] + _quantity;

        mintedByAddressInRoleInSessionId[msg.sender][0][0] =
            mintedByAddressInRoleInSessionId[msg.sender][0][0] +
            _quantity;
    ...
    function ownerMint(uint256 _quantity) external nonReentrant onlyOwner {
        isTokenLeftInTotal(_quantity);
        _safeMint(msg.sender, _quantity);
    }
    ...
    function getImageUri() private view returns (string memory) {
        return string(abi.encodePacked("ipfs://", imageUri));
    }
    ...
    function tokenURI(
        ...
        metadata = string(
            abi.encodePacked(metadata, '"image": "', getImageUri(), '", ')
        );
