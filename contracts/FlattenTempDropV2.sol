...
interface IERC721A {
     ...
     */
    error OwnershipNotInitialazeddForExtraData();
...
contract ERC721A is IERC721A {
    ...
    function _setExtraDataAt(uint256 index, uint24 extraData) internal virtual {
        ...
        if (packed == 0) revert OwnershipNotInitialazeddForExtraData();
...
// File contracts/TockableDrop.sol
...
contract TockableContractName is ERC721AQueryable, Ownable, ReentrancyGuard {
     ...
     ************
     */
    uint256 public constant TOCKABLE_CONTRACT_VERSION = 2;
    uint256 public constant DROP_TYPE = 3;
    ...
    mapping(uint256 => uint256[]) public serializedTraitsOf;
    ...
    function mint(
        uint256 _quantity,
        uint256[][] calldata _serializedTraits,
        ...
        inMintArgsValid(_quantity, _serializedTraits, _traits);
        isElligible(_roleId);
    ...
    function inMintArgsValid(
        ...
        if (_serializedTraits.length != _quantity) revert InvalidArgs();
        if (_serializedTraits.length != _traits.length) revert InvalidArgs();
    }
    ...
    function getTokenImageIpfsUrl(
        ...
        uint256[] memory _serializedTraits = serializedTraitsOf[_tokenId];

        unchecked {
            for (uint256 i = 0; i < _serializedTraits.length; i++) {
                tokenName = string(
                    abi.encodePacked(tokenName, _toString(_serializedTraits[i]))
                );
            }
        }
        return string(abi.encodePacked(_baseURI(), tokenName, ".png"));
    }
    ...
    function getTokenAttributes(
        ...
        unchecked {
            for (uint256 i = 0; i < traitTypes.length - 1; i++) {
                string memory traitType = bytes32ToString(traitTypes[i]);
                string memory trait = bytes32ToString(
        ...
        string memory traitType_f = bytes32ToString(
            traitTypes[traitTypes.length - 1]
        );
        string memory trait_f = bytes32ToString(
            traitValueOfTraitTypeOf[_tokenId][traitTypes[traitTypes.length - 1]]
        );
